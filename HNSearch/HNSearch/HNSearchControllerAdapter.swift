...
class HNSearchControllerAdapter: NSObject, UISearchResultsUpdating, UISearchBarDelegate {
    ...
    init(updateSearchBar: (UISearchBar) -> Void) {
        ...
        //let searchController = UISearchController(searchResultsController: nil)
        //searchResultsUpdater is a property on UISearchController that conforms to the new protocol UISearchResultsUpdating. This protocol allows your class to be informed as text changes within the UISearchBar. You’ll make the class conform to the protocol in a short while.
...
//        }
    ...
    private func search(text: String?, index: Int) {
        if let searchText = text, delegate = delegate {
            let options:[Tags?] = [nil,Tags.story,Tags.comment,Tags.ask_hn]
